...
## 2026-09-10 (cont'd) — Refactored the live-switch logic to async/await

Janet asked to switch to async/await to clean up the code logic. Converted `withTrackSwitchTimeout`, `verifyAudioTrackActive`, `selectAudioTrackForLanguage`, `switchLiveTrack`, `loadVideo()`'s automatic re-verify, and `handleLangSegmentClick()` (including the episode-card `confirmLanguageRestart()` path) from nested `.then()`/`.catch()` chains to `async`/`await`, flattening 3-4 levels of nesting down to mostly sequential statements. Extracted the select-vs-grace-period race (added earlier today for the English-revert hang fix) into a small named helper, `verifyAudioAfterSelect()`, using `Promise.race` instead of a manual `resolve()`-first-wins callback with its own dedup flag — the race primitive does that bookkeeping for free. `withTrackSwitchTimeout` itself stays a promise combinator (inherently a race between two independent timers, not a natural fit for `await`) but was similarly cleaned up into two named, self-logging racer promises instead of one manual `new Promise` with shared state. `handleLangSegmentClick()`'s `isPlaying && player` / else branch was also inverted into an early-return guard clause while converting, removing a level of nesting (behavior-preserving by De Morgan's law).

Deliberately unchanged: `player.pause()`/`.play()` stay bare fire-and-forget calls with `.catch()`, not `await`ed, since blocking on them was never the intent (see the original UX-feature plan's design decision on this).
...
## 2026-09-10 (cont'd) — Replaced live in-place language switching with the episode-card reload+modal pattern

After exhausting every client-side lever (see the many entries above) and confirming via a standalone reproduction that the same ~30% ceiling holds outside this codebase too (being filed as a Vimeo SDK bug), Janet decided to stop chasing live in-place track switching for the documentary and switch it to the same approach `.video-episode-block` already uses successfully: clicking a different language while playing now shows the existing `confirmLanguageRestart()` modal, and confirming reloads the iframe with the new language's URL params instead of trying to live-switch tracks on the existing player.

**`.video-quote-block` and `.video-episode-block` are now functionally identical** in `handleLangSegmentClick()`/`loadVideo()` (`video-toggle.js`) — deleted the entire `isQuoteBlock`-specific branch rather than leaving it inert: `logLangSwitch` and all of today's `[lang-switch]` debug logging, `TRACK_SWITCH_TIMEOUT_MS`/`AUDIO_SELECT_GRACE_MS`/`PLAYER_WARMUP_MS`/`waitForPlayerWarmup`, `withTrackSwitchTimeout`, `verifyAudioTrackActive`, `selectAudioTrackForLanguage`, `verifyAudioAfterSelect`, `switchLiveTrack`, the track-note functions, `setLangPickerBusy`, the loading-overlay functions, `showTrackSwitchStatus`, the `player`/`trackSwitchBusy` per-block state, and the now-unused `ensureVimeoSdk` import. `loadVideo()` no longer constructs a `Vimeo.Player` for quote blocks at all — the SDK script isn't even loaded for them anymore. Matching cleanup in `site-lang.php` (deleted the two now-dead template-registrar pairs; renamed `bitesmart_episode_needs_lang_restart_templates()` → `bitesmart_needs_lang_restart_templates()` since it now gates both block types, updated both real call sites plus the one in `episode-display.php`), `video-quote.php` (removed the loading-overlay markup and track-note paragraph, added the restart-templates call), `video-quote/style.css` and `shared/lang-picker.css` (removed the now-dead loading-overlay/spinner/disabled-segment rules). Two stray outdated doc-comment references (`shared-languages.js`, `site-lang.php`) fixed for accuracy while in the area.

Build size dropped noticeably (video-toggle.js: ~58KB → ~28.5KB compiled), consistent with how much of this was dead-weight-in-waiting once the live-switch path was gone.

**Tests** (`tests/videos/language-feedback.spec.js`, `tests/videos/loading.spec.js`): replaced the three live-switch-specific quote-block tests ("a fully successful live track switch...", "the live track-switch loading overlay markup...", "a disabled language segment does not register a click") with two tests mirroring the Episode section's own cancel/confirm-dialog tests, adapted for quote blocks' single `data-quote-vimeo-id` + `forceCaptions: true`. Rewrote "switching language while playing keeps the same video loaded" (which asserted the *old* behavior) to assert a reload now happens with the new language's URL params.

**Verified:** full `tests/videos` suite 24/24 (including the untouched Episode-section tests, confirming the shared code path didn't regress episode-card), `tests/analytics/events.spec.js` 14/14 (documentary-watched still fires correctly on both pages).

**Real finding from the manual spot-check (the plan's flagged "open question"):** the reload approach does *not* actually fix the underlying reliability problem. Checked ground truth directly (a second, independent `Vimeo.Player` reading `getAudioTracks()` after a real reload) across 6 real switches: **2/6 (33%) actually applied Spanish audio** — essentially the same failure rate as the abandoned live-switch API. `audiotrack=es` baked into a fresh embed hits the same underlying Vimeo-side problem as `selectAudioTrack()` did; it isn't a more mature/reliable mechanism after all. This is genuinely useful evidence for the GitHub issue (added there) — the same video's `es` track fails at a similar rate through two unrelated mechanisms, pointing at something video/account-side on Vimeo's end rather than just the new JS API being immature.
...
## 2026-09-11 — video-quote: separate per-language Vimeo videos instead of one multi-track video

Yesterday's reload+modal migration fixed the switching mechanics but exposed that the underlying problem is Vimeo's audio-track-*selection* reliability itself, not which API triggers it — the `audiotrack=` URL param hit the same ~30% ceiling as the live JS API (see yesterday's last entry). Janet's fix sidesteps the root cause entirely: she uploaded a genuinely separate Spanish-dubbed cut of the documentary to Vimeo as its own video (`1226008280`), using the existing video (`1169813728`) as the English one — the exact model `.video-episode-block` already uses successfully (one file per language, no track selection involved at all).

This also finished the `.video-quote-block`/`.video-episode-block` unification started yesterday. Quote blocks now emit the same `data-videos` JSON attribute episode blocks already did (instead of `data-quote-vimeo-id` + `data-supported-langs`), read by the same already-generic `resolveVideosForBlock()` — which meant `defaultQuoteLang()` and the `isQuoteBlock` branching in `loadVideo()`'s vimeoId lookup could both be deleted; `defaultEpisodeLang()` renamed to `defaultVideoLang()` since it now serves both block types. `forceCaptions: isQuoteBlock` is the only `isQuoteBlock` reference left anywhere in `video-toggle.js` — a real, deliberate UX difference, not a leftover.

**Block attributes**: kept `vimeoUrl` exactly as-is (name and meaning: "the English video"), matching what Janet asked for — added `vimeoUrlEs` alongside it rather than renaming to match episode-card's `vimeoUrlEn`, since a rename would have required rewriting every already-published quote block's saved content; this way nothing already live needed to change. Removed the `availableLanguages` checkbox attribute/UI entirely — availability is now simply "is this language's URL field filled in," derived the same way episode-card already derives it, removing a whole class of "checked the box but forgot the URL" (or vice versa) editor mistakes. New PHP: `bitesmart_video_quote_vimeo_ids_from_attrs()` (site-lang.php) mirrors `bitesmart_episode_vimeo_ids_from_attrs()` but reads the actual legacy pair (`vimeoUrl`/`vimeoUrlEs`, not `vimeoUrlEn`/`vimeoUrlEs`); `bitesmart_normalize_available_languages()` deleted (confirmed no other caller).

**Content rollout**: `vimeoUrlEs` is a per-block-instance attribute stored in each page's `post_content` — code alone doesn't retroactively add it to already-published blocks. Ran a one-off `wp_update_post()`/`parse_blocks()` script (same pattern as the `/learn` link fix, with Janet's explicit go-ahead on the method first) to add `vimeoUrlEs` to the documentary block on both pages it appears on (post 70, Home; post 2394, Learning hub) — verified by reading both back before and after, confirmed only that one key was added and nothing else changed.

**Verified**: full `tests/videos` suite, 22/22, including against the *real* separate Spanish video, not a mock. Also fixed a real bug caught while updating the tests: `loading.spec.js`'s reload test was asserting the post-switch iframe `src` against the *original* language's `vimeoId` — harmless when both languages shared one video, but wrong now that they're genuinely different videos; fixed to use the target language's own ID.

**Real finding from checking ground truth on the actual new Spanish video** (not just trusting the reload): `getAudioTracks()` showed *two* tracks both tagged `language: "es"` — a `kind: "main"` ("Original Audio", `enabled: true`, the one actually playing) and a `kind: "translation"` ("español", `enabled: false`). Captions were clean (one track, showing correctly). Flagged this to Janet rather than assuming it was fine. **Turned out to be a real content issue, not a code issue**: she'd uploaded the video without audio and added the Spanish track separately afterward (Vimeo's own "add an audio track" feature), rather than muxing the Spanish audio directly into the video file — which is exactly the kind of *separate-track* setup this whole change was meant to get away from, just now on a single-language upload instead of the original bilingual one. She's re-doing the upload with the audio properly baked in.
...
## 2026-09-11 — Fix dead "Read More" buttons on custom/article-or-commentary and custom/press-release

**What it does and why it was needed:** Janet reported that on `/news-media/#press-releases`, the "Read More" buttons under the `#articles-and-commentary` section did nothing when clicked.

**Root cause:** `read-more.js` (enqueued globally, shared by every expandable-card block) finds each `.expandable-article-block`'s own toggle/content pair with `article.querySelector(":scope > .read-more-toggle")` — a *direct-child* selector, added 2026-08-31 to stop a nested-books-list bug from grabbing the wrong (inner) button (see [[be-bitesmart-local-env]] history in the file's own comments). But `custom/article-or-commentary`'s and `custom/press-release`'s `save()` functions (in "text" mode) wrap `.expandable-content` and the button in an extra `<div>`, making them *grandchildren* of the article, not direct children — so `:scope >` never found them, `if (!toggle || !content) return;` bailed silently, and no click listener was ever attached. `custom/read-more` and `book-display.php`'s excerpt pattern were unaffected (both keep the button as a direct child already).

**Fix:** [plugins/custom-blocks-for-be-bite-smart/src/read-more.js](wp-content/plugins/custom-blocks-for-be-bite-smart/src/read-more.js) — replaced the `:scope >` lookups with a `findOwn()` helper that does `article.querySelectorAll(selector)` filtered to `el.closest(".expandable-article-block") === article`. This tolerates any wrapper depth (fixing the two broken blocks with zero markup/content changes) while still correctly excluding a nested inner `.expandable-article-block`'s own toggle/content (its `closest()` resolves to the inner article, not the outer one) — so the original 2026-08-31 books-list fix stays intact. Rebuilt via `pnpm build` (only `build/read-more.js` + its `.asset.php` version hash changed; every other bundle was untouched). Purged WP Super Cache locally so the new cache-busted script URL goes out immediately.

**Key decision:** Fixed the shared script rather than removing the extra wrapping `<div>` from the two blocks' `save()` functions. Editing `save()` would only fix *future* saves — it wouldn't retroactively unbreak whatever's already stored in `post_content` on live pages (Gutenberg static blocks bake their HTML into the post at save time), and would need every existing instance across the site re-saved to actually take effect, plus risks Gutenberg flagging existing content as an "invalid block" on next edit. Fixing `read-more.js` instead repairs every already-published instance immediately, with no content migration.

**Verification:** Playwright script against the live local site (`https://bebitesmart.local/news-media/`) — clicked the first Read More in `#articles-and-commentary`, confirmed the card's `.expandable-content` gained `.expanded`, the button's `data-expanded` flipped to `"true"`, and its label changed to "Read Less"; screenshotted the expanded card to confirm visually. `#press-releases` currently has no live "text"-mode press release (its one entry uses `expandableType: "pdf"`, a different code path with no Read More button), so that half of the bug has no live instance to demonstrate against right now, but is fixed by the same change.

**Commit:** `39ec737`.

**Regression test added:** Janet asked how to catch this automatically instead of relying on manual clicking — this exact class of bug (a button that renders fine but silently gets no click listener) had no coverage anywhere in the suite. Added [tests/smoke/read-more.spec.js](wp-content/tests/smoke/read-more.spec.js): scans every `CRITICAL_PAGES` route for visible `.read-more-toggle` buttons and clicks each one, asserting its own `.expandable-content` gains `.expanded`, `data-expanded` flips to `"true"`, and the label changes — then clicks again and asserts it collapses back (label and attribute restored), so a regression in either direction is caught. A toggle's own content is found via its immediate parent (`../*[...expandable-content...]`), which is correct whether that parent is the `.expandable-article-block` itself or the extra wrapper `<div>` this bug involved, and stays correctly scoped for `custom/books-list`'s nested excerpt case without needing to special-case it. Toggles hidden behind an unrelated "Show More" group (also `custom/books-list`) are skipped via `:visible`, since those aren't reachable by a user without a different, separately-tested click first.
...
## 2026-09-18 — Bio Card: photo floats so the short-bio text wraps around it

Janet asked for the `.bio-short` text to wrap around `.bio-photo` instead of the two sitting in separate side-by-side flex columns: photo stays to the left of name/role/affiliation/email/LinkedIn/bio-short, and once the running text passes the bottom of the photo it should widen to fill the space below it (standard float-wrap layout). "Read Full Biography" button left untouched.

**Changed:** `bio-card.php` — moved the photo's echo from a sibling of `.bio-details` (inside `.bio-section-1`) to the first child inside `.bio-details`, so the image is in the same flow as the text it should wrap around (a float only wraps text within its own block formatting context, not across flex siblings). `style.css` — removed `.bio-section-1`'s two-column flexbox (`display:flex`/`flex-direction`/`gap`, now dead since it wraps a single child) and `.bio-details`'s now-irrelevant `flex-grow`; added `.bio-details { display: flow-root }` so the box contains the floated photo instead of collapsing around it; added a `min-width: 768px` rule floating `.bio-photo` left with the same 4rem margin the old row gap used. Below that breakpoint the photo stays unfloated, rendering full-width via the existing generic `img` rule, preserving the original stacked mobile layout.

**Verified:** `pnpm run build` (wp-scripts) succeeded; confirmed the compiled `build/bio-card/style-index.css`/`style-index-rtl.css` picked up the new rules. Not verified in-browser — no browser/screenshot tool available this session, so the actual wrap behavior and mobile stacking still need a manual look in Local.

**Same day, follow-up**: Janet reported "Read Full Biography" (and by extension "Collapse Biography," sharing the same `.expanded-bio-toggle` class) had drifted left instead of staying centered now that `.bio-details` spans the full card width. Changed `.expanded-bio-toggle` from `display: inline-flex` to `display: flex; width: fit-content; margin: 1.5rem auto 0` (was `margin-top: 1.5rem` alone) so it shrink-wraps and horizontally centers under the wrapped text/photo instead of sitting flush left. Rebuilt (`pnpm run build`, clean).

**Same day, follow-up 2**: Janet noticed the short-bio wrap was inconsistent — sometimes hugging the photo, sometimes starting fully below it with visible white space that looked like it should've been usable. Explained the mechanism: the photo has a fixed height (up to 18rem via `aspect-ratio: 1` + `max-width`), while everything stacked above `.bio-short` (name, role, optional affiliation, email, contact-link buttons) varies in height per person — `.bio-contact-links`' own `2rem` top/bottom margins plus button height alone consumed roughly 110px of that 288px budget. Whatever's left over is what `.bio-short` gets to wrap into, so bios with more content above leave less (or no) room. Not a bug, but Janet asked to tighten it up: reduced `.bio-contact-links` margin from `2rem 0` to `1.25rem 0`, freeing ~24px more of the photo's height for the wrap on every card. Rebuilt (`pnpm run build`, clean). This narrows the problem, doesn't eliminate it — a card with a long enough name/role/affiliation stack can still exhaust the photo's height entirely; still unverified in-browser (no browser tool this session).

**Same day, follow-up 3**: since a CSS-only fix can't fully solve variable content height vs. a fixed photo height, Janet asked for a JS-measured fix: when there's less than one line's worth of room left beside the photo by the time `.bio-short` starts (an awkward sliver-width first line instead of a real wrap), push `.bio-short` below the photo entirely instead of letting it hug for a fraction of a line.

**Changed:** `bio-toggle.js` — new `adjustBioShortWrap()`, run on `DOMContentLoaded`, again on `document.fonts.ready` (web font swaps can shift text metrics after the first measurement), and on a debounced `resize` (only matters above the 768px breakpoint where the photo floats, per `style.css`). Per card, measures `photo.getBoundingClientRect().bottom` minus `bioShort.getBoundingClientRect().top` (the block's own top isn't affected by float-wrapping, only its inline content is, so this is a reliable "room beside the photo" figure regardless of the current wrap outcome) against `.bio-short`'s computed `line-height`; adds a `bio-short-below-photo` class when the room is positive but less than one line, clearing any previous verdict first so each run re-judges the browser's current attempt rather than compounding an old override. `style.css` — new `.bio-short.bio-short-below-photo { clear: left; }`, scoped so it only fires when the class is added.

**Verified:** `node --check` clean on the edited JS, `pnpm run build` clean, confirmed `bio-short-below-photo` landed in both the compiled `build/bio-card/bio-toggle.js` and `style-index.css`. Not verified in-browser — no browser/screenshot tool available this session; the actual measurement/threshold behavior across real bios (and the resize/font-swap re-runs) still needs a manual look in Local.

**Same day, follow-up 4**: Janet reported "Read Full Biography" sometimes sits to the right instead of centered on short bios (screenshot: Robin A. Grimm, PhD). Root cause: `.expanded-bio-toggle` is `display: flex`, and a flex container is itself treated as its own formatting context for the purposes of avoiding a sibling float — same reason `.bio-contact-links` correctly slots in beside the photo. On a short-enough bio the button's vertical position still falls within the photo's height, so it gets placed beside the photo (auto-centered only within that narrower leftover width) instead of clearing it and centering across the full card. Unlike `.bio-short`, this button was never meant to hug the photo at all, so the fix is simpler: added `clear: left` to `.expanded-bio-toggle` (shared by both "Read Full Biography" and "Collapse Biography") so it always starts below the photo before centering, regardless of bio length. Rebuilt (`pnpm run build`, clean).

**Not committed** — Janet commits manually per existing convention on this branch (`plc-updates`).
