...
registerBlockType("custom/hero", {
  ...
  // ── Deprecations ────────────────────────────────────────────────────────
  // Silently migrates any blocks that were saved with HTML before save()
  // was changed to return null. Without this, Gutenberg shows a
  // "Resolve Block" dialog every time an attribute changes.
  deprecations: [
    {
      attributes: {
        label: { type: "string", default: "Dog Bite Prevention" },
      ...
      // migrate passes attributes through unchanged — we just need
      // the old HTML to be accepted so the block stops erroring.
      migrate: (attributes) => attributes,
      save: () => null,
    },
  ],

  // ── save_UNUSED — kept for reference only, not called ──────────────────
  save_unused: ({ attributes }) => {
    ...
    const smUrl =
      ...
      attributes.bgImageMdUrl ||
      attributes.bgImageLgUrl;
    const mdUrl = attributes.bgImageMdUrl || attributes.bgImageLgUrl;
    const lgUrl = attributes.bgImageLgUrl;
    const hasAnyImage = !!(smUrl || mdUrl || lgUrl);
    ...
    return wp.element.createElement(
      ...
      wp.element.createElement(
        ...
        wp.element.createElement(
          ...
          // <picture> with per-breakpoint <source> elements.
          // Browser downloads only the first matching source — others never fetched.
          // Requires <picture> and <source> whitelisted in wp_kses_allowed_html.
          hasAnyImage &&
            wp.element.createElement(
              "picture",
              { className: "dbp-bg-picture", "aria-hidden": "true" },
              smUrl &&
