...
// Versions each enqueued stylesheet by hashing its own content, so the cache-busting
// query string (?ver=) actually changes whenever the CSS changes.
//
// This used to read the version out of the matching build/*.asset.php file instead
// (e.g. build/style.asset.php for style.css). Those files are generated by
// @wordpress/dependency-extraction-webpack-plugin from the webpack JS *entry*
// (src/style.js etc.) — a near-empty file that just does `import './style.css'` — not
// from the CSS itself. So that hash only changed when the JS entry changed, which is
// close to never; a CSS-only edit kept the same ?ver= and browsers/caches kept serving
// the old CSS indefinitely. (See CHANGES.md, 2026-08-25 entry, for how this was found.)
//
// filemtime() was considered and rejected for this same purpose: git doesn't preserve
// a file's original mtime, it stamps every file with the checkout time on `git pull` —
// so filemtime() would bust the cache for every stylesheet on every deploy, changed or
// not. Hashing the file's actual bytes only changes when the content actually does.
function get_css_version( $relative_path ) {
    $full = get_stylesheet_directory() . '/' . $relative_path;
    return file_exists( $full ) ? substr( md5_file( $full ), 0, 20 ) : '1.0.0';
}
...
// WordPress never actually loads the compiled JS files from build/ — the CSS files
// themselves are enqueued from their original paths (style.css, css/navbar.css etc.),
// versioned via get_css_version() above so cache-busting tracks the CSS content itself.
    ...
    wp_enqueue_style(
        ...
        get_css_version( 'css/navbar.css' )
    );
...
if ( is_page( 'contact' ) ) {
    wp_enqueue_style(
        ...
        array('twentytwentyfive-child'),
        get_css_version( 'css/forminator.css' )
    );
}
}
...
add_action( 'enqueue_block_assets', function() {
    wp_enqueue_style(
        ...
        array(),
        get_css_version( 'css/shared-block-styles.css' )
    );
});
